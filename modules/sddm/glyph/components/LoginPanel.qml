...
Item {
    ...
    SequentialAnimation {
        ...
        NumberAnimation {
            target: card
            property: "x"
            to: card.x - errorShake.distance
            duration: errorShake.duration
        ...
        NumberAnimation {
            target: card
            property: "x"
            to: (root.width - card.width) / 2
            duration: errorShake.duration
            easing.type: Easing.OutCubic
        }
        onStarted: {
            card.border.color = root.accentColor;
            errorTimer.start();
        }
    }
    ...
    Timer {
        id: errorTimer
        interval: 1000
        onTriggered: card.border.color = Qt.rgba(255, 255, 255, 0.3)
    }

    function handleLogin() {
        if (passwordField.text === "" || isLoggingIn) {
            triggerError();
            return;
        }
    ...
    Timer {
        id: loginTimeout
        interval: 5000
        onTriggered: root.isLoggingIn = false
    }
    ...
    Rectangle {
        ...
        radius: 22
        border.color: Qt.rgba(255, 255, 255, 0.0)
        border.width: 0
        ...
        ColumnLayout {
            ...
            MouseArea {
                ...
                onClicked: {
                    userDropdown.open = !userDropdown.open;
                    sessionDropdown.open = false;
                }
                RowLayout {
                    anchors.centerIn: parent
            ...
            Item {
                ...
                TextField {
                    ...
                    background: Rectangle {
                        ...
                        radius: 18
                        border.color: passwordField.activeFocus ? root.accentColor : Qt.rgba(255, 255, 255, 0.28)
                        border.width: 1
                    }
                ...
                Text {
                    text: root.showFailure ? "Incorrect password" : "Enter Password"
                    font.family: root.fontName
                    font.pixelSize: 14
                    color: root.showFailure ? "#FF453A" : Qt.rgba(255, 255, 255, 0.5)
                    anchors.centerIn: parent
            ...
            RowLayout {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        sessionDropdown.open = !sessionDropdown.open;
                        userDropdown.open = false;
                    }
                    Rectangle {
                        anchors.fill: parent
                        ...
                        RowLayout {
                            ...
                            spacing: 8
                            Text {
                                text: root.activeSessionName.toUpperCase()
                                font.family: root.fontName
                                font.pixelSize: 12
                                color: root.textColor
                                opacity: 0.7
                ...
                Button {
                    ...
                    enabled: !root.isLoggingIn
                    background: Rectangle {
                        color: root.isLoggingIn ? Qt.rgba(255, 255, 255, 0.1) : root.accentColor
                        radius: 25
                        scale: loginButton.pressed ? 0.9 : 1.0
                    }
                    contentItem: Item {
                        Text {
                            anchors.centerIn: parent
                            text: root.isLoggingIn ? "⋯" : "→"
                            color: "white"
                            font.pixelSize: 28
                        }
                    }
        ...
        Rectangle {
            ...
            height: Math.min(200, (userModel ? userModel.count : 0) * 50 + 20)
            ...
            ListView {
                ...
                delegate: MouseArea {
                    ...
                    Text {
                        anchors.centerIn: parent
                        text: (model.name || model.display || "USER").toUpperCase()
                        font.family: root.fontName
                        font.pixelSize: 16
                        color: userList.currentIndex === index ? root.accentColor : Qt.rgba(255, 255, 255, 0.6)
                    }
                }
            }
        }
        ...
        Rectangle {
            ...
            z: 100
            width: parent.width - 20
            height: Math.min(200, (sessionModel ? sessionModel.count : 0) * 50 + 20)
            anchors.bottom: parent.bottom
            ...
            ListView {
                ...
                Keys.onDownPressed: incrementCurrentIndex()
                Keys.onReturnPressed: {
                    root.sessionSelected(currentIndex);
                    sessionDropdown.open = false;
                    root.focusPassword();
                }
                delegate: MouseArea {
                    ...
                    Text {
                        anchors.centerIn: parent
                        text: cleanName(model.name || model.display || "SESSION").toUpperCase()
                        font.family: root.fontName
                        font.pixelSize: 14
                        color: sessionList.currentIndex === index ? root.accentColor : Qt.rgba(255, 255, 255, 0.6)
                    }
                }
            }
        }
