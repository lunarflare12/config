...
PanelWindow {
    ...
    // Hyprland will not tear or direct-scanout while another surface is on the
    // same monitor. Unmap the bar whenever a Steam/gamescope client is
    // fullscreen — not only when it is the active toplevel. XWayland games
    // often leave activeToplevel stale, which kept the 2560x666 layer mapped
    // and capped the game to compositor vsync.
    readonly property bool gameFullscreen: {
        ...
        for (let i = 0; i < values.length; i++) {
            if (root.isFullscreenGame(values[i]))
                return true;
        }
        return root.isFullscreenGame(Hyprland.activeToplevel);
    }
    ...
    function isFullscreenGame(t) {
        ...
        if (typeof fs === "number")
            ...
            on = fs.some(function (v) { return Number(v) > 0; });
        if (!on)
            return false;
    ...
    exclusiveZone: root.gameFullscreen ? -1 : Core.Theme.pillHeight + margins.top + 2
    ...
    readonly property int surfaceTop: 6
    ...
    readonly property real targetWidth: root.popupShape ? root.activeLauncher.cardWidth : root.barContentWidth + 16
