...
Singleton {
    ...
    readonly property bool softwareGamma: root.device === "hypr"
    ...
    property FileView backlightFile: FileView {
        path: root.device === "" || root.softwareGamma ? "" : "/sys/class/backlight/" + root.device + "/actual_brightness"
    ...
    function refresh() {
        if (root.device === "" || root.softwareGamma)
            root.refreshReading();
        else
    ...
    property Timer poll: Timer {
        interval: root.interacting ? 200 : 1500
        running: root.device !== ""
        repeat: true
